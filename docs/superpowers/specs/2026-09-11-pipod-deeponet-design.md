...
- **基流**：球体势流解析解经各向异性映射到椭球：`ξ = M(x−c)`（M=diag(1/a,1/b,1/c)），`u(x) = M⁻¹ · û(ξ)`，û 为**来流取 V = U·M·d̂** 的单位球势流速度场（这样远场才严格有 u → M⁻¹V = U·d̂；若 û 直接取 U·d̂，远场会变成 U·(d̂⊙axes)，非球轴比下与来流条件矛盾——实施阶段实测修正）。性质：**连续性严格成立**（常对角映射下 `div_x u = Σ_i M⁻¹_ii·M_ii·∂û_i/∂ξ_i = div_ξ û = 0`）；**椭球面滑移壁面严格成立**（u·n ∝ û·ξ = 0）；
