...
Source: https://github.com/xie-lab-ml/multiphysics-bench/main/FNO/merge_data_NS_heat.py
Author: @xie-lab-ml
"""
...
mat_dir_train = 'training/NS_heat/'
output_file_train = 'merge_data/NS_heat_train_128.pt'
os.makedirs(os.path.dirname(output_file_train), exist_ok=True)

mat_dir_test = "testing/NS_heat/"
output_file_test = 'merge_data/NS_heat_test_128.pt'
...
range_allQ_heat_paths = f"{mat_dir_train}Q_heat/range_allQ_heat.mat"
...
for idx in range(num_samples_train):
    # Q_heat
    path_Q_heat = os.path.join(f"{mat_dir_train}Q_heat/", f'{idx+1}.mat')
    ...
    y_data[idx, 0] = u_u_normalized.astype(np.float32)
    y_data[idx, 1] = u_v_normalized.astype(np.float32)


    # T
    path_T = os.path.join(f"{mat_dir_train}T/", f'{idx+1}.mat')
...
for idx in range(num_samples_test):
    # Q_heat 
    path_Q_heat = os.path.join(f"{mat_dir_test}Q_heat/", f'{idx+10001}.mat')
    Q_heat = io.loadmat(path_Q_heat)['export_Q_heat']
    Q_heat_normalized = (Q_heat - min_Q_heat) / (max_Q_heat - min_Q_heat) * 1.8 - 0.9 # [-0.9,0.9]
    ...
    # T 
    path_T = os.path.join(f"{mat_dir_test}T/", f'{idx+10001}.mat')
    T = io.loadmat(path_T)['export_T']
    T_normalized = (T - min_T) / (max_T - min_T) * 1.8 - 0.9 # [-0.9,0.9]
